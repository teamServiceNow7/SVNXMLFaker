...
def main():
    ...
    if uploaded_files:
        ...
        if selected_file:
            ...
            if usage:
                with st.sidebar.expander("#### Update Idle Duration"):
                    st.markdown("")
                ...
                with st.sidebar.expander("#### Session Duration"):
                    st.markdown("")
                ...
                if idle_dur_date and idle_dur_time:
                    total_idle_dur = datetime.combine(idle_dur_date, idle_dur_time)
                else:
                    total_idle_dur = None            
                if session_dur_date and session_dur_time:        
            ...
            if update_button:
                ...
                # Simulating updating the file in the uploader
                # We need to re-upload the modified file, which is not directly possible.
                # However, we can simulate this by showing the updated content and prompting user action.
                st.write("Updated file content:")
                st.text_area("Updated XML Content", value=modified_xml.decode('utf-8'), height=300) 
